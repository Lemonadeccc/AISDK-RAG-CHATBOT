  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessage({ type: 'error', text: result.error || 'An error occurred while processing the PDF' })
      }
      ...
      <Card className='mb-6'>
        <CardContent className="pt-6">
          <div className="space-y-4">
            ...
            <Input id="pdf-upload" type="file" accept=".pdf" onChange={handleFileUpload} disabled={isLoading} className="mt-2" />
          </div>
          {isLoading && (
